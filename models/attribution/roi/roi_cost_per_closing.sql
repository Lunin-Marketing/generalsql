...
WITH campaign_base AS (
    ...
    FROM opp_base
    WHERE closing_date >= campaign_member_first_responded_date
    ...
    LEFT JOIN {{ref('demand_gen_costs_fy23')}}
        ...
        AND DATE_TRUNC('month',opp_base.closing_date)=DATE_TRUNC('month',demand_gen_costs_fy23.start_date)
    ...
    SELECT DISTINCT
        ...
        cost AS total_cost,
        COUNT(DISTINCT opportunity_id) AS closing
