...
WITH campaign_base AS (
    ...
    FROM person_base
    WHERE working_date >= campaign_member_first_responded_date
    ...
    LEFT JOIN {{ref('demand_gen_costs_fy23')}}
        ON person_base.channel_bucket_lt=demand_gen_costs_fy23.channel_bucket
        AND DATE_TRUNC('month',person_base.working_date)=DATE_TRUNC('month',demand_gen_costs_fy23.start_date)
    ...
    SELECT DISTINCT
        ...
        cost AS total_cost,
        COUNT(DISTINCT person_id) AS sals
