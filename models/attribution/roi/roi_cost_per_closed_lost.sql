...
WITH campaign_base AS (
    ...
    WHERE opportunities_with_contacts.close_date IS NOT null
    AND opportunities_with_contacts.stage_name IN ('Closed – Lost No Resources / Budget','Closed – Lost Not Ready / No Decision','Closed – Lost Product Deficiency','Closed - Lost to Competitor', 'Closed Lost')
    ...
    FROM opp_base
    WHERE close_date >= campaign_member_first_responded_date
    ...
    LEFT JOIN {{ref('demand_gen_costs_fy23')}}
        ...
        AND DATE_TRUNC('month',opp_base.close_date)=DATE_TRUNC('month',demand_gen_costs_fy23.start_date)
