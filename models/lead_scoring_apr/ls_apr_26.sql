...
WITH email_click_base AS (
    ...
    AND message_click_date >= CURRENT_DATE-93
    AND message_click_date <= CURRENT_DATE-3

), ao_forms AS (

    SELECT *
    FROM {{ref('ao_forms_xf')}}
    WHERE form_date_time >= CURRENT_DATE-93
    AND form_date_time <= CURRENT_DATE-3
    ...
    WHERE beacon_datetime >= CURRENT_DATE-93
    AND beacon_datetime <= CURRENT_DATE-3
    ...
    AND webinar_registration_date_time >= CURRENT_DATE-93
    AND webinar_registration_date_time <= CURRENT_DATE-3
    ...
    AND webinar_start_date_time >= CURRENT_DATE-93
    AND webinar_start_date_time <= CURRENT_DATE-3
    ...
    FROM {{ref('ao_ctp_xf')}}
    WHERE "When" >= CURRENT_DATE-93
    AND "When" <= CURRENT_DATE-3
