...
WITH email_click_base AS (
    ...
    AND message_click_date >= CURRENT_DATE-117
    AND message_click_date <= CURRENT_DATE-27

), ao_forms AS (

    SELECT *
    FROM {{ref('ao_forms_xf')}}
    WHERE form_date_time >= CURRENT_DATE-117
    AND form_date_time <= CURRENT_DATE-27
    ...
    WHERE beacon_datetime >= CURRENT_DATE-117
    AND beacon_datetime <= CURRENT_DATE-27
    ...
    AND webinar_registration_date_time >= CURRENT_DATE-117
    AND webinar_registration_date_time <= CURRENT_DATE-27
    ...
    AND webinar_start_date_time >= CURRENT_DATE-117
    AND webinar_start_date_time <= CURRENT_DATE-27
