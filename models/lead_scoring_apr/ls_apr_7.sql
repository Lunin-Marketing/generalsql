...
WITH email_click_base AS (
    ...
    AND message_click_date >= CURRENT_DATE-112
    AND message_click_date <= CURRENT_DATE-22

), ao_forms AS (

    SELECT *
    FROM {{ref('ao_forms_xf')}}
    WHERE form_date_time >= CURRENT_DATE-112
    AND form_date_time <= CURRENT_DATE-22
    ...
    WHERE beacon_datetime >= CURRENT_DATE-112
    AND beacon_datetime <= CURRENT_DATE-22
    ...
    AND webinar_start_date_time >= CURRENT_DATE-112
    AND webinar_start_date_time <= CURRENT_DATE-22
    ...
    FROM {{ref('ao_ctp_xf')}}
    WHERE "When" >= CURRENT_DATE-112
    AND "When" <= CURRENT_DATE-22
