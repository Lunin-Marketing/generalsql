...
WITH email_click_base AS (
    ...
    AND message_click_date >= CURRENT_DATE-104
    AND message_click_date <= CURRENT_DATE-14

), ao_forms AS (

    SELECT *
    FROM {{ref('ao_forms_xf')}}
    WHERE form_date_time >= CURRENT_DATE-104
    AND form_date_time <= CURRENT_DATE-14
    ...
    WHERE beacon_datetime >= CURRENT_DATE-104
    AND beacon_datetime <= CURRENT_DATE-14
    ...
    AND webinar_start_date_time >= CURRENT_DATE-104
    AND webinar_start_date_time <= CURRENT_DATE-14
    ...
    FROM {{ref('ao_ctp_xf')}}
    WHERE "When" >= CURRENT_DATE-104
    AND "When" <= CURRENT_DATE-14
