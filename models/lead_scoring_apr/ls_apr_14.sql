...
WITH email_click_base AS (
    ...
    AND message_click_date >= CURRENT_DATE-105
    AND message_click_date <= CURRENT_DATE-15

), ao_forms AS (

    SELECT *
    FROM {{ref('ao_forms_xf')}}
    WHERE form_date_time >= CURRENT_DATE-105
    AND form_date_time <= CURRENT_DATE-15
    ...
    WHERE beacon_datetime >= CURRENT_DATE-105
    AND beacon_datetime <= CURRENT_DATE-15
    ...
    AND webinar_start_date_time >= CURRENT_DATE-105
    AND webinar_start_date_time <= CURRENT_DATE-15
    ...
    FROM {{ref('ao_ctp_xf')}}
    WHERE "When" >= CURRENT_DATE-105
    AND "When" <= CURRENT_DATE-15
