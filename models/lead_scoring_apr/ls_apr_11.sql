...
WITH email_click_base AS (
    ...
    AND message_click_date >= CURRENT_DATE-108
    AND message_click_date <= CURRENT_DATE-18

), ao_forms AS (

    SELECT *
    FROM {{ref('ao_forms_xf')}}
    WHERE form_date_time >= CURRENT_DATE-108
    AND form_date_time <= CURRENT_DATE-18
    ...
    WHERE beacon_datetime >= CURRENT_DATE-108
    AND beacon_datetime <= CURRENT_DATE-18
    ...
    AND webinar_start_date_time >= CURRENT_DATE-108
    AND webinar_start_date_time <= CURRENT_DATE-18
    ...
    FROM {{ref('ao_ctp_xf')}}
    WHERE "When" >= CURRENT_DATE-108
    AND "When" <= CURRENT_DATE-18
