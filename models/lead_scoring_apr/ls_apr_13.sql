...
WITH email_click_base AS (
    ...
    AND message_click_date >= CURRENT_DATE-106
    AND message_click_date <= CURRENT_DATE-16

), ao_forms AS (

    SELECT *
    FROM {{ref('ao_forms_xf')}}
    WHERE form_date_time >= CURRENT_DATE-106
    AND form_date_time <= CURRENT_DATE-16
    ...
    WHERE beacon_datetime >= CURRENT_DATE-106
    AND beacon_datetime <= CURRENT_DATE-16
    ...
    AND webinar_start_date_time >= CURRENT_DATE-106
    AND webinar_start_date_time <= CURRENT_DATE-16
    ...
    FROM {{ref('ao_ctp_xf')}}
    WHERE "When" >= CURRENT_DATE-106
    AND "When" <= CURRENT_DATE-16
