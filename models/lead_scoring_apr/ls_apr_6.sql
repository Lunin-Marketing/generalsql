...
WITH email_click_base AS (
    ...
    AND message_click_date >= CURRENT_DATE-113
    AND message_click_date <= CURRENT_DATE-23

), ao_forms AS (

    SELECT *
    FROM {{ref('ao_forms_xf')}}
    WHERE form_date_time >= CURRENT_DATE-113
    AND form_date_time <= CURRENT_DATE-23
    ...
    WHERE beacon_datetime >= CURRENT_DATE-113
    AND beacon_datetime <= CURRENT_DATE-23
    ...
    AND webinar_registration_date_time >= CURRENT_DATE-113
    AND webinar_registration_date_time <= CURRENT_DATE-23
    ...
    AND webinar_start_date_time >= CURRENT_DATE-113
    AND webinar_start_date_time <= CURRENT_DATE-23
    ...
    FROM {{ref('ao_ctp_xf')}}
    WHERE "When" >= CURRENT_DATE-113
    AND "When" <= CURRENT_DATE-23
